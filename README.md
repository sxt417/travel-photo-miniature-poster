...
将旅行照片重建为具有实体建筑模型摄影质感的 1:1 微缩沙盘海报。Skill 会分析照片中的地标、建筑、地形、植被、水体和空间关系，并生成带地下剖面与右上角无文字场景平面图的方形海报。
...
- 固定输出 1:1 正方形，并使用锁定的标题、沙盘尺寸、安全边距和暖象牙背景。
...
```text
使用 $travel-photo-miniature-poster 将这张旅行照片制作成 1:1 实体建筑微缩沙盘海报。
```
